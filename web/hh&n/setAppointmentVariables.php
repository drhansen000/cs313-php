<?php
    ...
    $stmt2 = $db->prepare('SELECT employee.name, service, date, time, cost FROM service JOIN appointment ON service.id = appointment.serviceID JOIN person AS employee ON employee.id = appointment.employeeID JOIN person AS customer ON customer.id = appointment.customerID WHERE customer.id=:theid ORDER BY date');
